...
public class DBInsertFinancialRatios extends DBConnectMySql {

	private final Logger logger = (Logger) LoggerFactory.getLogger(DBInsertFinancialRatios.class);
	...
	public void prepDataForInsert (String fileCompanyFinancialData, String dbName, String tblName){
		Map<String, List<String>> mapFinancialRatiosData = new HashMap<String, List<String>> ();
		...
		else
		{
			try{
				...
				mapFinancialRatiosData  = parseIndividualStmtData(inputList, "Cash Flow");
				Set<String> keys = mapFinancialRatiosData.keySet();
				...
				for (String symbol: keys){
					List<String> listData = mapFinancialRatiosData.get(symbol);
					...
					for (int i = 2; i <=5; i++){
						...
						for (String line: listData){
							...
							else if (temp[0].equals(("Period Ending:").trim())){
								quarter[i-2] = utils.generateQuarterGivenDate(temp[i]);
								/*
								if (temp[i].equals("12/31/2017")){
									quarter[i-2] = "FY2017";
								}
								else if (temp[i].equals("12/31/2016")){
									quarter[i-2] = "FY2016";
								}
								else if (temp[i].equals("12/31/2015")){
									quarter[i-2] = "FY2015";
									//logger.info("Set quarter to:"+"2Q2017");
								}
								else if (temp[i].equals("12/31/2014")){
									quarter[i-2] = "FY2014";
									//logger.info("Set quarter to:"+"1Q2017");
								}
								else if (temp[i].equals("12/31/2013")){
									quarter[i-2] = "FY2013";
